...
export default function AdminResults({ settings }: Props) {
  ...
  const loadCompletedEvents = async () => {
    try {
      // The Live Race Console uses heats as the source of completed/raced events.
      // We intentionally do NOT depend on the separate results table for the selector.
      ...
      (data || []).forEach((heat: any) => {
        ...
        if (event && heat.event_id && !unique.has(String(heat.event_id))) {
          unique.set(String(heat.event_id), event as SwimEvent);
        }
      });

      setCompletedEvents(Array.from(unique.values()));
    } catch (error) {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">View results for completed swimming events</p>
      </div>
        ...
        <select value={selectedEventId} onChange={(e) => setSelectedEventId(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-slate-300 outline-none focus:border-cyan-500 bg-white">
          ...
          {completedEvents.map((ev) => (
            <option key={ev.id} value={ev.id}>
              {ev.category || 'Event'} - {ev.event_name}
            </option>
          ))}
        </select>
      ...
      {!selectedEventId ? (
        <div className="text-center py-20 bg-white rounded-2xl shadow-md">
          ...
          <p className="text-slate-500">Select a completed event to view its results.</p>
        </div>
      ) : loading ? (
